...
IPEDS_Race <- left_join(IPEDS_Race,
                          Carnegie_Classification %>%
                            select(unitid,Carnegie_Classifcation2021),
                          join_by = "unitid")
...
```{r Philadelphia Maps}
Universities %>%
  group_by(NAME, CARNEGIE_CLASSIFCATION2021, RACE) %>%
  summarize(FREQUENCY = sum(FREQUENCY)) %>%
  filter(RACE == "Total_Enrollment") %>%
  ggplot() +
  geom_sf( aes(color = CARNEGIE_CLASSIFCATION2021,
               size = FREQUENCY)) +
  geom_sf(data = Philadelphia_School_District,
          fill = "transparent") +
  mapTheme()
...
ggplot(aes(x = RACE, y = Total, fill = RACE)) +
